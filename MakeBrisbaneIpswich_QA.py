...
def main(argv):
    ...
    if not workingpath==None:
        ...
        orig=AtlassGen.makedir(os.path.join(workingpath,'orig'))
        norm=AtlassGen.makedir(os.path.join(workingpath,'norm'))
    ...
    normlaz=os.path.join(tempworkspace,'{0}_norm.laz'.format(tile))
    ...
    laztests.append(['_default_near',-0.5,2,[1],None])
    laztests.append(['_unclassified',-10,150,[0],None])
    laztests.append(['_bld_near',-0.5,2,[6],None])
    ...
    for test in laztests:
        ofilenorm=os.path.join(norm,'{0}{1}.laz'.format(tile,test[0]))
        ofile=os.path.join(orig,'{0}{1}.laz'.format(tile,test[0]))
        low=test[1]
        high=test[2]
        classes=test[3]
