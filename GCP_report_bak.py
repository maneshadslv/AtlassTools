...
def main():
    ...
    for key, gcp in originalpoints.items():
        ...
        if pointstat:
            if not found:
                ...
                data_all_txt = data_all_txt + '{0} {1} {2} {3} {4}\n'.format(point.dz, point.lidarz, point.x, point.y, point.controlz)
                idxyz_txt = idxyz_txt + '{0} {1} {2} {3}\n'.format(point.index, point.x, point.y, point.controlz)
    ...
    txtfile = os.path.join(outputpath,'CGP_all_accepted.txt').replace('\\','/')
    with open(txtfile, 'w') as txtf:
        txtf.write(data_all_txt)
    
    #Write the id,x,y.z txt file for all the accepted patches, to be used with tin method
    if pointstat:
        idxyztxtfile = os.path.join(outputpath,'CGP_accepted.IDXYZ').replace('\\','/')
        with open(idxyztxtfile, 'w') as idtxtf:
            idtxtf.write(idxyz_txt)
