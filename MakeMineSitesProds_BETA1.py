...
def param_parser():
    ...
    main_parser.add_argument("aoifolder", metavar="AOI Folder", widget="DirChooser", help="Select folder with Product aoi shape files", default='')
    main_parser.add_argument("gndclasses", metavar = "Ground Classes", default="2 8")
    main_parser.add_argument("cores",metavar="Cores", help="No of cores to run in", default=8)
...
def makeMKPperTile(tilename,xmin,ymin,xmax,ymax,tilesize, inputfolder, mkp_working, main_workingdir, gndclasses,vt, hz, buffer,tl_in, filetype,aois,makeEMB):
    ...
    if makeEMB == False:
        ...
        bufferedfile= os.path.join(buffered_dir,'{0}.{1}'.format(tilename,filetype)).replace('\\','/')
    ...
    if os.path.exists(bufferedfile):
        ...
        outfile = os.path.join(mkp_working,'{0}.laz'.format(tilename)).replace('\\','/')
        ...
        try:
            ...
            prodclippeddir = AtlassGen.makedir(os.path.join(mkp_working, 'clipped')).replace('\\','/')
            ...
            for aoi in aois:
                ...
                AtlassGen.clip(outfile, clippedoutput, aoi, 'laz')
...
def makeEmbossperTile(tl_in, tl_out,xmin,ymin,xmax,ymax,tilename,tilesize,inputfolder,workingdir,filetype,hydropointsfiles,buffer,gndclasses, step, kill,aoifiles,prjfile,gsd):
...
def main(argv):
    ...
    aoifolder = args.aoifolder
    aoifiles = AtlassGen.FILELIST(['*.shp'],aoifolder)
    print('{1} AOIS found : {0}'.format(aoifiles, len(aoifiles)))
    epsg = args.epsg
    ...
    outputdir = AtlassGen.makedir(os.path.join(outputpath, '{0}_Submission'.format(dt))).replace('\\','/')
    ...
    for tile in tl_out: 
        ...
        if makeEMB:
            tte = '{0}_{1}'.format(tilename,'EMB')
            make_PRODS[tte] = AtlassTask(tilename, makeEmbossperTile, tl_in, tl_out, xmin,ymin,xmax,ymax,tilename,tilesize,inputfolder,workingdir,filetype,hydropointsfiles,buffer, gndclasses, step, kill,aoifiles,prjfile,gsd)
    ...
    print(len(make_PRODS))
    p=Pool(processes=cores)        
    make_dems_results=p.map(AtlassTaskRunner.taskmanager,make_PRODS.values())
    ...
    for tile in tl_out: 
        ...
        if makeMKP:
            ...
            make_PRODS[ttm] = AtlassTask(tilename,makeMKPperTile, tilename,xmin,ymin,xmax,ymax,tilesize, inputfolder, mkpdir_work, workingdir, gndclasses,vt, hz, buffer,tl_in, filetype,aoifiles,makeEMB)
    ...
    for aoi in aoifiles:
        ...
        proddir_dest = AtlassGen.makedir(os.path.join(outputdir,aoiname)).replace('\\','/')

        if makeEMB:


            print('Merging DEM for AOI : {0}'.format(aoiname))
            ...
            mergedlasfile = os.path.join(mergeddir,"{0}_Emboss_Image.laz".format(aoiname)).replace('\\','/')
            mergedascfile = os.path.join(mergeddir,"{0}_Emboss_Image.asc".format(aoiname)).replace('\\','/')
            copy_file = os.path.join(proddir_dest,"{0}_Emboss_Image.asc".format(aoiname)).replace('\\','/')
            mergedpngfile = os.path.join(proddir_dest,"{0}_Emboss_Image.jpg".format(aoiname)).replace('\\','/')
            prj = os.path.join(proddir_dest,"{0}_Emboss_Image.prj".format(aoiname)).replace('\\','/')
...
UNLOAD_ALL'''.format(prj,copy_file, mergedpngfile)
        ...
        if makeMKP:
            ...
            vtv = int(vt*100)           
            mergedlasfile = os.path.join(proddir_dest,"{0}_MKP_{1}cm.laz".format(aoiname,vtv)).replace('\\','/')
            mergedtxtfile = os.path.join(proddir_dest,"{0}_MKP_{1}cm.txt".format(aoiname,vtv)).replace('\\','/')
            mkpdir_work = os.path.join(workingdir, 'MKP_{0}m'.format(vt)).replace('\\', '/')
            prodclippeddir = os.path.join(mkpdir_work, 'clipped').replace('\\','/')
            aoidir_work = os.path.join(prodclippeddir, aoiname).replace('\\','/')

            print(aoidir_work, proddir_dest)
            merge_product_mkp(aoidir_work, mergedlasfile, mergedtxtfile,makeTXT)
