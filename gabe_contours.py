...
class ContourClass:
    def makecontourprocess(tilename,inputfolder,gms_path,bufferedout_path,bufferremoved_path,clippedout_path, buffer,contourinterval,zone,aois,index,hydropointsfiles,gmexe,tilelayoutfile,prjfile):
        ...
        try:
            #Make Contour Files
            ContourClass.makecontours(tilename,neighbourfiles, buff_output, tile,  int(buffer), contourinterval)    
            ...
            for aoi in aois:    
                ...
                ContourClass.makegmsfiles(tilename,bufferedout_path,gms_path,bufferremoved_path,aoipath,tile, str(zone), aoi, index,tilelayoutfile,gmscript)
    ...
    def makegmsfiles(filename,inpath,outpath,buffoutpath,clipoutpath,tile,zone, AOI_clip, index,tilelayoutfile,dstfile):
        ...
        template = "\\\\10.10.10.142\\projects\\PythonScripts\\templates\\Template.gms"
    ...
    def makecontours(filename,neighbourfiles, output, tile,buffer, contourinterval):
        ...
        try:
            ####MANESHA - CHANGED from blas2iso.exe to last2iso64.exe - 02/09/2019 ####
            # blas2iso does not do a good job in steaper areas. Gab found issue that had bow ties and touching contours
            ############################################################################################################################
            subprocessargs=['C:/LAStools/bin/blas2iso.exe','-i'] + neighbourfiles + ['-merged','-oshp', '-iso_every', contourinterval,'-clean',4,'-o',output,'-kill', 1000] + keep 
            subprocessargs=list(map(str,subprocessargs))
            p = subprocess.run(subprocessargs,stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False, check=True, universal_newlines=True)
