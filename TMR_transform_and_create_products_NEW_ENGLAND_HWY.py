...
def main():
    ...
    for til in tilelayout:

        tilename = til.name
        file = os.path.join(inputfolder,"{0}.{1}".format(tilename, filetype))

        #finalnames[filename]={}
        
        output = os.path.join(adjdir, '{0}.{1}'.format(tilename, filetype)).replace("\\", "/")
     
        adj_tasks[tilename] = AtlassTask(tilename, adjust, file, output, dx, dy, dz, epsg, filetype)
    ...
    for result in mkp_index_results:
        ...
        if result.success:
            ...
            output = os.path.join(lasahddir, '{0}_AHD_C2_{5}_MGA94_{3}_SW{1}_{2}.{4}'.format(areaname, x, y, zone, 'las',year)).replace("\\", "/") #<inputpath>/Products/<Area_Name>_LAS_AHD/<Name>_2018_2_AHD_SW_<X>m_<Y>m_<Zone>_1k.las
    ...
    for result in  clip_demlaz_results:
        ...
        if result.success:
            ...
            output=os.path.join(dem1esridir,'{0}_DEM_1m_{3}_MGA94_56_SW_{1}_{2}.asc'.format(areaname, x, y, year)).replace('\\','/')
    ...
    for result in lastoasciigrid_results:
        ...
        if result.success:
            ...
            #files 
            input=os.path.join(dem1esridir,'{0}_DEM_1m_{3}_MGA94_56_SW_{1}_{2}.asc'.format(areaname, x, y, year)).replace('\\','/') #dtm asci file
            prjfile1 = os.path.join(dem1esridir,'{0}_DEM_1m_{3}_MGA94_56_SW_{1}_{2}.prj'.format(areaname, x, y, year)).replace('\\','/')
            shutil.copyfile(projfile, prjfile1) 
            output = os.path.join(dem1dir,'{0}_DEM_1m_{3}_MGA94_56_SW_{1}_{2}.xyz'.format(areaname, x, y, year)).replace('\\','/')
