        ...
        for result in makebuff_results:
            ...
            if result.success:
                ...
                if product =='CHM':
                    ...
                    output=os.path.join(proddir,'{0}_{1}.asc'.format(tilename, product)).replace('\\','/')
                    tasks[tilename] = AtlassTask(tilename, MakeCHM, int(x), int(y), input, chmgridfile, output, chmdemgridfile, tilename, buffer, kill, step, chmstep, int(tilesize), filetype)
