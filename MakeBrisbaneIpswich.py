...
defaults['hydropoints']="//10.10.10.142/processed_data/BR01280_Brisbane_to_Ipswich-DNRME/01_LiDAR/05_Product_generation/BrisbaneIpswichLiDAR2019/Synthetic_Hydro_LAZ/merged_flat.laz" #hydro points file for hydro flattening
...
def main(argv):
    ...
    ---------------------------------------------------------------------------------------------------------------------------------------
    '''
    #temp workspace for classified tiles
    temporiginaltiles_classified=AtlassGen.makedir(os.path.join(tempworkspace,'original_classified'))
    copyneighbourtiles(xmin,ymin,xmax,ymax,buffer,tilelayout_500m_tl,filelocations,temporiginaltiles_classified,prefix='ws_',extn='.laz')
    #add files to cleanup
    cleanupfolders.append(temporiginaltiles_classified)

    #merge orginal tiles with buffer +50 
    mergedclassified=os.path.join(tempworkspace,'01_mergedclassified.laz')
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i','{0}/*.laz'.format(temporiginaltiles_classified),'-merged','-olaz','-o',mergedclassified,'-set_version','1.2','-epsg',28356]
    ...
    PrintMsg('merged classified created -- {0}'.format(os.path.isfile(mergedclassified)),Heading=True)
    ...
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i','{0}/*.laz'.format(temporiginaltiles),'-merged','-olaz','-o',mergedoriginal,'-set_version','1.2','-set_classification',0,'-epsg',28356]
    subprocessargs=subprocessargs+['-reoffset',xorigin,yorigin,0]
    subprocessargs=subprocessargs+['-keep_xy',xmin-buffer,ymin-buffer,xmax+buffer,ymax+buffer]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    '''
    ---------------------------------------------------------------------------------------------------------------------------------------
    copy classification and check
    ---------------------------------------------------------------------------------------------------------------------------------------
    '''
    lascopied=os.path.join(tempworkspace,'03_lascopied.laz')
    subprocessargs=['C:/LAStools/bin/lascopy64.exe','-i',mergedclassified,'-i',mergedoriginal,'-olaz','-classification','-o',lascopied]
    subprocessargs=map(str,subprocessargs)
    subprocess.call(subprocessargs)  
    PrintMsg('lascopied created -- {0}'.format(os.path.isfile(lascopied)),Heading=True)

    #output a laz file with any points left in class 0 for manual check
    unclassified=os.path.join(os.path.join(workingtiles,'{0}_unclassified.laz'.format(tile)))
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i',lascopied,'-olaz','-o',unclassified,'-keep_class',0]
    subprocessargs=subprocessargs+['-keep_xy',xmin,ymin,xmax,ymax]
    subprocessargs=map(str,subprocessargs)
    subprocess.call(subprocessargs)  
    PrintMsg('unclassified created -- {0}'.format(os.path.isfile(unclassified)),Heading=True)
    ...
    #---------------------------------------------------------------------------------------
    # 01_LAS_AHD
    las_ahd='SW_{0}_{1}_1k_class_AHD.laz'.format(int(xmin),int(ymin))
    las_ahd=os.path.join(AtlassGen.makedir(os.path.join(workingpath,'01_LAS_AHD')),las_ahd)
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i',lascopied,'-olaz','-o',las_ahd]
    subprocessargs=subprocessargs+['-keep_xy',xmin,ymin,xmax,ymax]
    subprocessargs=map(str,subprocessargs)
    subprocess.call(subprocessargs)
    ...
    las_ell='SW_{0}_{1}_1k_class_Ellipsoid.laz'.format(int(xmin),int(ymin))
    las_ell=os.path.join(AtlassGen.makedir(os.path.join(workingpath,'02_LAS_Ellipsoid')),las_ell)

    subprocessargs=['C:/LAStools/bin/lasheight64.exe','-i',las_ahd,'-olaz','-o',las_ell,'-ground_points',tilegeoid, '-all_ground_points']
    subprocessargs=subprocessargs+['-keep_xy',xmin,ymin,xmax,ymax]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',las_ahd,'-otif','-nbits',8,'-o',Intensity,'-step',1,'-intensity_average','-first_only']
    ...
    subprocessargs=['C:/LAStools/bin/blast2dem.exe','-i',DEM_esri,'-olaz','-rescale',0.001,0.001,0.001,'-o',demlaz]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',demlaz,'-otif','-nbits',32,'-o',DEM_tif32,'-step',1,'-elevation_lowest']
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-nrows',1000,'-ncols',1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',las_ahd,'-oasc','-nbits',32,'-o',dsm_raw,'-step',1,'-elevation_highest','-keep_class',1,3,4,5,6,8,10,11,13,14,15,16,17,18,19]
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-nrows',1000,'-ncols',1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/blast2dem.exe','-i',DSM_esri,'-olaz','-rescale',0.001,0.001,0.001,'-o',dsmlaz]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',dsmlaz,'-otif','-nbits',32,'-o',DSM_tif32,'-step',1,'-elevation_lowest']
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-nrows',1000,'-ncols',1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i',hydropoints,'-olaz','-o',tilehydro,'-set_version','1.2','-set_classification',2,'-epsg',28356]
    subprocessargs=subprocessargs+['-reoffset',xorigin,yorigin,0]
    subprocessargs=subprocessargs+['-keep_xy',xmin-1000,ymin-1000,xmax+1000,ymax+1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/blast2dem.exe','-i',DEM_esri,'-olaz','-rescale',0.001,0.001,0.001,'-o',demlaz_hydro]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',demlaz_hydro,'-otif','-nbits',32,'-o',DEM_tif32,'-step',1,'-elevation_lowest']
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-nrows',1000,'-ncols',1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/blast2dem.exe','-i',DSM_esri,'-olaz','-rescale',0.001,0.001,0.001,'-o',dsmlaz_hydro]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid64.exe','-i',dsmlaz_hydro,'-otif','-nbits',32,'-o',DSM_tif32,'-step',1,'-elevation_lowest']
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-nrows',1000,'-ncols',1000]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    subprocessargs=['C:/LAStools/bin/las2las64.exe','-i',lascopied,'-olaz','-o',contourpointslaz,'-keep_class',2,19]
    subprocessargs=subprocessargs+['-keep_xy',xmin-buffer,ymin-buffer,xmax+buffer,ymax+buffer]
    subprocessargs=map(str,subprocessargs) 
    subprocess.call(subprocessargs)  
    ...
    buff=interval*0.3 #-filtervalue (must not be more than 30% of interval)
