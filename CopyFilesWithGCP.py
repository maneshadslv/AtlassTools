    ...
    if args.command == "TIN_method":
        ...
        if not genreport:
            ...
            for tile in GCPTile.keys():
                ...
                points=GCPTile[tile]
                inputtile = os.path.join(inputfolder,'{0}.laz'.format(tile)).replace('\\','/')
                outputtile = os.path.join(outputpath,'{0}.laz'.format(tile)).replace('\\','/')
                shutil.copy(inputtile,outputtile)
