...
def main(argv):
    ...
    final=os.path.join(deliverypath,'HQP_2019_C2_{0}_tocolour.laz'.format(tile))
	image_path = os.path.join(RGB, 'HQP_2019_C2_{0}.tif'.format(tile))
	
	coloured = os.path.join(deliverypath,'HQP_2019_C2_{0}_coloured.laz'.format(tile))
	...
	subprocessargs=['C:/LAStools/bin/lascolor.exe','-i',final,'-image', image_path,'-olaz','-o',coloured]
	

    if os.path.isfile(coloured):
