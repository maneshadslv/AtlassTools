...
class DEMClass():
    ...
    def makeDEMperTile(tilename,inputdir,outputdir,workingdir,hydropoints,inputgeojsonfile,outputgeojsonfile,filetype,outputfilename,gndclass,buffer,kill,step,clipp,aois):
        ...
        workingdir = AtlassGen.makedir(os.path.join(workingdir, 'DEM')).replace('\\','/')
        buffFile = os.path.join(buff_dir,'{0}.{1}'.format(tilename,filetype)).replace('\\','/')

        DEMoutputdir =  AtlassGen.makedir(os.path.join(outputdir, 'makeGRID_output/DEM')).replace('\\','/')
        ...
        try:
            ...
            if clipp:
                ...
                for aoi in aois:
                    ...
                    print(aoiname)
                    aoidir = AtlassGen.makedir(os.path.join(workingdir, aoiname).replace('\\','/'))
                    aoioutdir = AtlassGen.makedir(os.path.join(DEMoutputdir, aoiname).replace('\\','/'))
...
class DSMClass():
    ...
    def makeDSMperTile(tilename,inputdir,outputdir,workingdir,inputgeojsonfile,outputgeojsonfile,filetype,outputfilename,nongndclasses,buffer,kill,step,clipp,aois):
        ...
        nongndclasses = nongndclasses.split(' ')
        ...
        demfile = os.path.join(outputdir, 'makeGRID_output/DEM/{0}.asc'.format(outputfilename)).replace('\\','/')
        print(buffered_inputfile)


        inputfile = AtlassGen.bufferTile(tile,tl_in,buffered_inputfile,buffer,nongndclasses,inputdir,filetype)[1]


        DSMoutputdir =  AtlassGen.makedir(os.path.join(outputdir, 'makeGRID_output/DSM')).replace('\\','/')
        DSMworkdir = AtlassGen.makedir(os.path.join(workingdir, 'DSM')).replace('\\','/')
        ...
        try:
            ...
            if clipp:
                ...
                for aoi in aois:
                    pathn, aoiname,ext = AtlassGen.FILESPEC(aoi)
                    print(aoiname)
                    aoidir = AtlassGen.makedir(os.path.join(DSMworkdir, aoiname).replace('\\','/'))
                    aoioutdir = AtlassGen.makedir(os.path.join(DSMoutputdir, aoiname).replace('\\','/'))
                    clippedlaz = os.path.join(aoidir,'{0}_clipped.laz'.format(outputfilename)).replace('\\','/')
