...
def param_parser():
    ...
    FCM_parser.add_argument("--aoifiles", metavar="AOI shp file", widget="MultiFileChooser", help="Select aoi shape file/s for clipping", default='')
...
def makeProductsperTile(tl_in, tl_out,xmin,ymin,xmax,ymax,tilename,tilesize,productlist,inputfolder,workingdir,outputdir,filetype,hydropointsfiles,buffer,nongndclasses, gndclasses, chmclasses, hydrogridclasses, makeDEM, makeDSM, makeHYDRO, makeCHM, step, chmstep,kill,clipshape,aoifiles,prjfile,merged,projname,gsd,chmgsd,makeTIFF):
    ...
    for product in productlist:
        ...
        prjfile1 = os.path.join(proddir,'{0}{1}_{2}_{3}.prj'.format(proj,product,gsd,tilename)).replace('\\','/')
        output = os.path.join(proddir,'{0}{1}_{2}_{3}.asc'.format(proj,product,gsd,tilename)).replace('\\','/')

        #TIF Files
        if makeTIFF:
            deletefiles.append(output)
            outputTIF = output.replace('.asc','.tif')
            outputTFW = output.replace('.asc','.tfw')
            deletefiles.append(outputTIF)
            deletefiles.append(outputTFW)
        ...
        if makebuff_results[0]:
            ...
            if product =='DSM':
                ...
                demfile = os.path.join(outputdir,'DEM_{4}m/{0}DEM_{1}_{2}.asc'.format(proj,gsd,tilename,tilesize,step)).replace('\\','/')
                ...
                for aoi in aoifiles:
                    ...
                    if os.path.isfile(lasoutput):
                        ...
                        if product == 'CHM':
                            step = chmstep
                            gsd = chmgsd
                        ascoutput=os.path.join(aoidir,'{0}{1}_{2}_{3}.asc'.format(proj,product,gsd,tilename,tilesize)).replace('\\','/')
                        prjfile1 = os.path.join(aoidir,'{0}{1}_{2}_{3}.prj'.format(proj,product,gsd,tilename,tilesize)).replace('\\','/')


                        lastoasciigrid(int(xmin), int(ymin), lasoutput, ascoutput, int(tilesize), step)
                        deletefiles.append(ascoutput)
                        #product_files.append(prjfile1)
                        shutil.copyfile(prjfile, prjfile1) 
