...
def main(argv):
    ...
    subprocessargs=['C:/LAStools/bin/lasgrid.exe','-i',mergedlas,'-olaz','-o',dsmtempfile2,'-nbits',32,'-elevation_lowest','-step',step,'-subcircle',step,'-fill',1,'-keep_class'] + [2,8]
    subprocessargs=map(str,subprocessargs)        
    subprocess.call(subprocessargs)    
    ...
    subprocessargs=['C:/LAStools/bin/blast2dem.exe','-i',dsmtempfile2,'-oasc','-o',dsmtempfile3,'-nbits',32,'-step',step,'-kill',kill]
    subprocessargs=subprocessargs+['-ll',xmin,ymin,'-ncols',math.ceil((xmax-xmin)/step), '-nrows',math.ceil((ymax-ymin)/step)]
    subprocessargs=map(str,subprocessargs)        
    subprocess.call(subprocessargs)    
    ...
    nonvoids_dem=AsciiGrid() 
    nonvoids_dem.header=a.header
    nonvoids=ones*(a.grid!=a.nodata_value)
    dist=morphology.distance_transform_cdt(nonvoids,metric='chessboard').astype(nonvoids.dtype)
    nonvoids_dem.grid=np.where(np.logical_or(dist==1,hydrovoids==1),b.grid,nodata)
    nonvoids_demfile=mergedlas.replace('.laz','_HYDRO_Voids_Height2.asc')
    
    nonvoids_dem.savetofile(nonvoids_demfile) 
