...
def param_parser():
    ...
    tin_parser.add_argument("--cores", metavar="General", help="Number of cores to be used", type=int, default=4)
    ...
    patchstat_parser.add_argument("--cores", metavar="General", help="Number of cores to be used", type=int, default=4)
...
def main():
    ...
    if args.command == "TIN_method":
        ...
        dt = strftime("%d/%m/%y")
        logo = 	"\\\\10.10.10.142\\projects\\PythonScripts\\icons\\Aerometrex_logo_FINAL_new.png"
        ...
        statdata = 'GCP points sampled: {0}\nGCP points accepted: {1}({2}%)\nGCP points rejected: {3}({4}%)\nOut of Range: {5}({6}%)\nMean Average: {7}\nRMSE after z shift applied: {8}\nCI95 after z shift applied: {9}'.format(points, points_accept, points_accept_perc, points_rej, points_rej_perc,gcp_outof_range,gcp_outof_range_perc,pointlist.finalaverage,rmse,ci95)
        statfile = os.path.join(outputpath,'stats.txt').replace('\\','/')
        with open(statfile, 'w') as stf:
            stf.write(statdata)

        shiftdata = 'X:{0}\nY:{1}\nZ:{2}'.format(deltax, deltay,shift)
        shiftfile = os.path.join(outputpath,'shifts.txt').replace('\\','/')
        with open(shiftfile, 'w') as shf:
            shf.write(shiftdata)
