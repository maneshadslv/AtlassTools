...
defaults={}
defaults['filetoremove']=None #file to zip and remove
...
def main(argv):
    ...
    #python \\10.10.10.142\projects\PythonScripts\remove_file.py --filetoremove=#outpath#\#name#.las 
    ...
    filetoremove=settings['filetoremove']
    if not filetoremove==None:
        filetoremove=filetoremove.replace('\\','/')
        if os.path.isfile(filetoremove):
            os.remove(filetoremove)
        else:
            PrintMsg('no file {}'.format(filetoremove))
    else:
        PrintMsg('filetoremove not set')
        return
