...
sys.path.append('{0}/lib/shapefile_original.original/'.format(sys.path[0]).replace('\\','/'))
...
def main():
    ...
    for file in files:
        ...
        if os.path.isfile(file):
            ...
            pathw,tilename,ext = AtlassGen.FILESPEC(file)
            print(tilename)
            print(pathw)
            #files
            input = os.path.join(pathw,'{0}.{1}'.format(tilename, filetype)).replace('\\','/')  #adjusted las
            ...
            for neighbour in neighbours:
                neighbour = os.path.join(pathw,'{0}.{1}'.format(neighbour, filetype)).replace('\\','/')
    ...
    mkp_tasks = {}
    for file in files:
            pathw,filename,ext = AtlassGen.FILESPEC(file)
            print(filename)
            x,y = filename.split("_")
    ...
    ''' -------------Keep This commented for the rest  -------------------
