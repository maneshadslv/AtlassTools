...
def genLasinfo(lazfile,outputpathtxt):
    ...
    try:
        ...
        p = subprocess.run(subprocessargs,stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False, check=True, universal_newlines=True,timeout=15)  
...
def rename_swath(swathlazfile,wdpfilepath,outputpath,basename,psid,zone):
    ############################################################################
    attribs={}
    ...
    lasinfofile = os.path.join(outputpathtxt,'c_{0}_{1}.txt'.format(basename, psid)).replace('\\','/')
    ...
    wfile = wdpfile.replace('export - Channel 1 - {0}_Channel_1 - originalpoints.wdp'.format(basename),'WesternWoodlands2021-UNC-ELL-{0}_{5}_{1}{2}_{3}_{4}.wdp'.format(psid,int(minx),int(miny),width,height,zone))
    print(wfile)
...
if __name__ == "__main__":
    ...
    lazfile,wdpfilepath,outputpath,basename,psid,zone = sys.argv[1:7]
    ...
    rename_swath(lazfile,wdpfilepath,outputpath,basename,psid,zone)
